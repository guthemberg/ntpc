...
# os ultimos dois argumentos do scripts a seguir distiguem src, dst, prt e ip, exemplo, o primeiro eh IP src
  csh ${libexec}/hauptsache.sh kartei.txt $tipo ${root} 0 0 $libexec
  csh ${libexec}/hauptsache.sh kartei.txt $tipo ${root} 0 1 $libexec
  csh ${libexec}/hauptsache.sh kartei.txt $tipo ${root} 1 0 $libexec
  csh ${libexec}/hauptsache.sh kartei.txt $tipo ${root} 1 1 $libexec
end 
