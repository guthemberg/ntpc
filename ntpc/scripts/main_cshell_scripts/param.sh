...
if ( !( $#argv == 3 ) ) then
  ...
  echo "opt: indica a opcao de processamento: p2p, bit, edk, kaz, aut. Eh importante lembrar que all refere-se as colunas 8-11 dos arquivos p2p_TS*. Lembre-se, aqui vc deverah ter o arquivo all jah processado."
  exit
endif
set root = ${1}
if ( -d $root ) then
...
# os ultimos dois argumentos do scripts a seguir distiguem src, dst, prt e ip, exemplo, o primeiro eh IP src
  csh ${libexec}/param1.sh param.txt $tipo ${root} 0 0 $libexec
  csh ${libexec}/param1.sh param.txt $tipo ${root} 0 1 $libexec
  csh ${libexec}/param1.sh param.txt $tipo ${root} 1 0 $libexec
  csh ${libexec}/param1.sh param.txt $tipo ${root} 1 1 $libexec
end 
