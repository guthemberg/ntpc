...
# os ultimos dois argumentos do scripts a seguir distiguem src, dst, prt e ip, exemplo, o primeiro eh IP src
  csh ${libexec}/principal.sh fichier.txt $bc $root 0 0 $libexec $opt
  csh ${libexec}/principal.sh fichier.txt $bc $root 0 1 $libexec $opt 
  csh ${libexec}/principal.sh fichier.txt $bc $root 1 0 $libexec $opt
  csh ${libexec}/principal.sh fichier.txt $bc $root 1 1 $libexec $opt
  @ bc++
end
