...
if ( !( $#argv == 3 ) ) then
  ...
  echo "opt: indica a opcao de processamento: p2p, bit, edk, kaz, aut, all. Eh importante lembrar que all refere-se as colunas 8-11 do arquivo TS "
  exit
endif
set root = ${1}
if ( -d $root ) then
...
set table = ( $opt )
foreach opcao ( $table[*] )
  csh ${libexec}/main.sh file.txt $opcao ${root} 0 0 $libexec
  csh ${libexec}/main.sh file.txt $opcao ${root} 0 1 $libexec
  csh ${libexec}/main.sh file.txt $opcao ${root} 1 0 $libexec
  csh ${libexec}/main.sh file.txt $opcao ${root} 1 1 $libexec
end 
